...
### new with temporarily fixed cosine similarity 
import os
import sys
...
def main():
    ...
    if len(res_ls) == 0:
        print("No candidates passed threshold; skipping selection.")
        return

    # Clamp selection count to available items
    select_num = min(args.num, len(res_ls))
    ...
    # text_emb_np has shape (len(res_ls), D); ensure we don't ask sklearn with 0 rows
    if text_emb_np.shape[0] == 0:
        print("Empty text embedding matrix; skipping selection.")
        return

    res = cosine_similarity(text_emb_np, text_emb_target_np).reshape(-1)

    # Take top candidates (up to 300), then sample up to select_num safely
    top_k = min(300, len(res))
    candidate = np.argsort(res)[::-1][:top_k]
    if len(candidate) == 0:
        print("No candidates after similarity; skipping selection.")
        return

    k = min(select_num, len(candidate))
    random_selected_candidate = random.sample(list(candidate), k)
